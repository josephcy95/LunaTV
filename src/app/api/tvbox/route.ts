...
function getBaseUrl(request: NextRequest): string {
  ...
  const forwardedPort = (request.headers.get('x-forwarded-port') || '')
    ...
    .trim();
  const protocol =
    forwardedProto || requestUrl.protocol.replace(':', '') || 'http';
  let host =
    forwardedHost ||
...
export async function GET(request: NextRequest) {
  try {
        ...
        enabledSources.map(async (source) => {
          ...
          if (proxyConfig?.enabled && proxyConfig.proxyUrl) {
            ...
            if (isPrivateOrLocalApiUrl(realApiUrl)) {
              finalApi = realApiUrl;
              console.log(
                `[TVBox Proxy] ${source.name}: skipped (LAN/Tailscale/private API)`,
              );
            } else {
    ...
    if (customJarToUse) {
      ...
      console.log(
        `[Spider] 自定义 jar: ${customJarUrl}${customJarFromConfig ? ' (全局配置)' : ' (源站配置)'}，通过代理提供`,
      );
