...
// Query Options 工厂函数
const embySourcesOptions = (embyEnabled: boolean) =>
  queryOptions({
...
const embyViewsOptions = (embyEnabled: boolean, embyKey?: string) =>
  queryOptions({
...
const embyListOptions = (
  ...
  infiniteQueryOptions({
    ...
    queryFn: async ({ pageParam, signal }) => {
      ...
      const res = await fetch(`/api/emby/list?${params.toString()}`, {
        signal,
      });
...
const embySearchOptions = (
  ...
  queryOptions({
    ...
    queryFn: async ({ signal }) => {
      ...
      const res = await fetch(`/api/emby/search?${params.toString()}`, {
        signal,
      });
    ...
    enabled:
      embyEnabled && !!embyKey && (searchKeyword?.trim().length ?? 0) > 0,
    staleTime: 60 * 1000,
    gcTime: 5 * 60 * 1000,
  });
...
export default function PrivateLibraryPage() {
  ...
  const {
    ...
    embyListOptions(embyEnabled, embyKey, selectedView, sortBy, sortOrder),
  );
  ...
  const errorMessage = isError
    ? (listError as Error)?.message || '获取列表失败，请稍后重试'
    : '';
        ...
        <div className='mb-6'>
          <div className='flex items-center justify-between mb-4'>
            ...
            >
              ...
              <RefreshCw
                className={`h-4 w-4 relative z-10 ${isRefreshing ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'}`}
              />
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      localStorage.setItem('emby_sortBy', option.value);
                    }}
