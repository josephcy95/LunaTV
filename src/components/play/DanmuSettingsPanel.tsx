...
export const DanmuSettingsPanel = memo(function DanmuSettingsPanel({
  ...
  const [sliderArea, setSliderArea] = useState(() =>
    areaIndexFromMargin(settings.margin),
  );
  const [sliderDensity, setSliderDensity] = useState(() =>
    clampDanmuDensity(settings.density),
  );
  ...
  useEffect(() => {
    ...
    setSliderArea(areaIndexFromMargin(settings.margin));
    setSliderDensity(clampDanmuDensity(settings.density));
  }, [
    settings.fontSize,
    settings.speed,
    settings.opacity,
  ...
  const commitArea = useCallback(() => {
    const nextMargin = marginFromAreaIndex(sliderArea);
    if (
      String(nextMargin[0]) !== String(settings.margin[0]) ||
      String(nextMargin[1]) !== String(settings.margin[1])
    ) {
      handleUpdate('margin', nextMargin);
    }
  }, [handleUpdate, settings.margin, sliderArea]);

  const commitDensity = useCallback(() => {
    const next = clampDanmuDensity(sliderDensity);
    if (next !== settings.density) {
      handleUpdate('density', next);
    }
  }, [handleUpdate, settings.density, sliderDensity]);
        ...
        {isManualOverridden && settings.enabled && (
          ...
          >
            <div className='min-w-0'>
              <p className='text-xs text-amber-300 font-medium'>手动覆盖模式</p>
              ...
              <div className='flex items-center gap-3'>
                ...
                <div className='relative flex-1'>
                  <input
                    ...
                    value={sliderArea}
                    onChange={(e) =>
                      setSliderArea(parseInt(e.target.value, 10))
                    }
                    onMouseUp={commitArea}
                    onTouchEnd={commitArea}
                    onBlur={commitArea}
                    className='w-full h-2 rounded-full appearance-none cursor-pointer transition-all'
                    style={{
                      background: `linear-gradient(to right, #10b981 0%, #10b981 ${(sliderArea / 5) * 100}%, rgba(75, 85, 99, 0.5) ${(sliderArea / 5) * 100}%, rgba(75, 85, 99, 0.5) 100%)`,
                    }}
                  />
                </div>
                <span className='text-xs text-green-400 w-12 text-right font-mono font-semibold tabular-nums'>
                  {DANMU_AREA_STEPS[sliderArea]?.label ?? '1/6'}
                </span>
              </div>
              ...
              <div className='flex items-center gap-3'>
                ...
                <div className='relative flex-1'>
                  <input
                    ...
                    value={sliderDensity}
                    onChange={(e) =>
                      setSliderDensity(parseInt(e.target.value, 10))
                    }
                    onMouseUp={commitDensity}
                    onTouchEnd={commitDensity}
                    onBlur={commitDensity}
                    className='w-full h-2 rounded-full appearance-none cursor-pointer transition-all'
                    style={{
                      background: `linear-gradient(to right, #10b981 0%, #10b981 ${((sliderDensity - 1) / 5) * 100}%, rgba(75, 85, 99, 0.5) ${((sliderDensity - 1) / 5) * 100}%, rgba(75, 85, 99, 0.5) 100%)`,
                    }}
                  />
                </div>
                <span className='text-xs text-green-400 w-12 text-right font-mono font-semibold tabular-nums'>
